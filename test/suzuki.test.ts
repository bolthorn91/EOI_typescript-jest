...
import {stairsInNYears, stepCounter} from '../src/suzuki';

stepCounter
jest.mock('../src/suzuki', () => ({
  stepCounter: jest.fn()
}))

describe("stairsInNYears", function() {

  it("should return the sum of all stairs on each day", function() {
  ...
  expect(stairsInNYears(stairs, 20)).toBe(54636040);

  test('should return 0 when years is a negative number', () => {
    expect(stairsInNYears(stairs, -1)).toBe(0);
  })

  test('should return sum stairs on each year when years are float', () => {
    expect(stairsInNYears(stairs, 5.5)).toBe(15024911);
  })
